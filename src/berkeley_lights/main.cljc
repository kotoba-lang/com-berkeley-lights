(ns berkeley_lights.main
  "Kotodama WASM entrypoint for the Berkeley_lights clean-room actor (L4) — Clojure port.
  ...
  facts (`berkeley_lights.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "berkeley_lights")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"berkeley_lights.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "berkeley_lights-compat" :tier tier :entities entities} 200])
...
(defn start! [] :berkeley_lights-compat/ready)
